import { Deal, Message } from "./types";

export const MOCK_DEALS: Deal[] = [
  {
    id: "DEAL-IR-2024-001",
    title: "Steel Export — Tata to Severstal",
    sector: "Metals & Mining",
    parties: "Tata Steel, Severstal, Sber BD India",
    status: "on_track",
    riskLevel: "Low",
    confidenceScore: 0.92,
    bottlenecks: [],
    suggestedTasks: ["Monitor shipment documentation", "Verify LC terms"],
    lastActivity: "2 hours ago",
    value: "$4.2M",
    progress: 75,
  },
  {
    id: "DEAL-IR-2024-002",
    title: "Pharmaceutical Supply — Sun Pharma to R-Pharm",
    sector: "Pharmaceuticals",
    parties: "Sun Pharma, R-Pharm, Sber BD India",
    status: "action_needed",
    riskLevel: "Medium",
    confidenceScore: 0.74,
    bottlenecks: ["Regulatory approval pending from CDSCO"],
    suggestedTasks: ["Follow up on CDSCO clearance", "Prepare alternative shipping route"],
    lastActivity: "45 minutes ago",
    value: "$8.7M",
    progress: 45,
  },
  {
    id: "DEAL-IR-2024-003",
    title: "Oil Refinery Equipment — L&T to Rosneft",
    sector: "Energy",
    parties: "L&T, Rosneft, Sber BD India",
    status: "blocked",
    riskLevel: "High",
    confidenceScore: 0.58,
    bottlenecks: ["Sanctions compliance review required", "Insurance coverage gap"],
    suggestedTasks: ["Escalate to compliance team", "Source alternative insurer"],
    lastActivity: "15 minutes ago",
    value: "$22.5M",
    progress: 20,
  },
  {
    id: "DEAL-IR-2024-004",
    title: "IT Services Contract — Infosys to Sberbank",
    sector: "Technology",
    parties: "Infosys, Sberbank, Sber BD India",
    status: "on_track",
    riskLevel: "Low",
    confidenceScore: 0.89,
    bottlenecks: [],
    suggestedTasks: ["Schedule next milestone review"],
    lastActivity: "1 hour ago",
    value: "$3.1M",
    progress: 60,
  },
  {
    id: "DEAL-IR-2024-005",
    title: "Agricultural Commodities — ITC to United Grain",
    sector: "Agriculture",
    parties: "ITC Limited, United Grain Company, Sber BD India",
    status: "action_needed",
    riskLevel: "Medium",
    confidenceScore: 0.67,
    bottlenecks: ["Quality certification delayed"],
    suggestedTasks: ["Contact FSSAI for expedited cert", "Negotiate extended delivery window"],
    lastActivity: "30 minutes ago",
    value: "$6.3M",
    progress: 35,
  },
  // Synthetic Data from Qdrant
  {
    id: "IND-RUS-2026-0042",
    title: "Pharmaceutical Settlement — MedCorp Moscow to BioPharma Mumbai",
    sector: "Pharmaceuticals",
    parties: "MedCorp Moscow, BioPharma Mumbai, Sberbank",
    status: "blocked",
    riskLevel: "High",
    confidenceScore: 0.65,
    bottlenecks: ["Rupee-Ruble settlement rate disagreement", "FX hedging required"],
    suggestedTasks: ["Schedule rate negotiation call", "Propose Sberbank hedging product FX-Option-B"],
    lastActivity: "3 hours ago",
    value: "$12.5M",
    progress: 15,
  },
  {
    id: "SBER-IND-RU-2026-002",
    title: "Generic Antibiotics — Main Chain LLC to Sun Pharma",
    sector: "Pharmaceuticals",
    parties: "Main Chain LLC, Sun Pharma / Biosintez, Sber BD India",
    status: "action_needed",
    riskLevel: "Medium",
    confidenceScore: 0.78,
    bottlenecks: ["Delays in mandatory Russian certification", "eBRC documentation pending"],
    suggestedTasks: ["Use GigaChat AI for documentation verification", "Expedite electronic Bank Realisation Certificate"],
    lastActivity: "2 hours ago",
    value: "$15.3M",
    progress: 40,
  },
  {
    id: "SBER-IND-RU-2026-003",
    title: "Agriculture Logistics — LLC Unifrost to Marine Exports India",
    sector: "Logistics / Agriculture",
    parties: "LLC Unifrost, Marine Exports India, Sberbank",
    status: "action_needed",
    riskLevel: "Medium",
    confidenceScore: 0.81,
    bottlenecks: ["Suez Canal to INSTC Eastern Route transition", "Transit insurance coordination"],
    suggestedTasks: ["Confirm Rupee surplus interest rate 7%", "Apply 20-40% transit tariff discount"],
    lastActivity: "1 day ago",
    value: "$8.2M",
    progress: 55,
  },
  {
    id: "SBER-IND-RU-2026-004",
    title: "Packaging Chemicals — Stupino Kvadrat SEZ to UFlex Group",
    sector: "Industrial Packaging / Chemicals",
    parties: "Stupino Kvadrat SEZ, Flex Films Rus / UFlex Group, Sberbank",
    status: "action_needed",
    riskLevel: "Medium",
    confidenceScore: 0.72,
    bottlenecks: ["Capital account transaction approval for RUB 4.6B facility"],
    suggestedTasks: ["Match with Russian chemical partners for ink sourcing", "Verify zero-rated VAT status"],
    lastActivity: "2 days ago",
    value: "$18.7M",
    progress: 35,
  },
  {
    id: "SBER-IND-RU-2026-005",
    title: "Steel Commodities — Northern Steel LLC to Tata Steel",
    sector: "Metals / Commodities",
    parties: "Northern Steel LLC, Tata Steel India, Sberbank",
    status: "on_track",
    riskLevel: "Medium",
    confidenceScore: 0.87,
    bottlenecks: ["INR-RUB settlement timing", "eBRC documentation for tax incentives"],
    suggestedTasks: ["Confirm contract currency clause", "Initiate letter of credit discounting"],
    lastActivity: "30 minutes ago",
    value: "$19.4M",
    progress: 70,
  },
  {
    id: "SBER-IND-RU-2026-006",
    title: "Crude Oil Settlement — Reliance Industries to Rosneft",
    sector: "Energy / Oil & Gas",
    parties: "Reliance Industries, Rosneft, Sberbank",
    status: "action_needed",
    riskLevel: "High",
    confidenceScore: 0.76,
    bottlenecks: ["Ruble-based settlement pilot setup", "Sberbank-Gazprombank payment corridor establishment"],
    suggestedTasks: ["Finalize ruble payment corridor for 500,000 bpd", "Ensure eBRC issuance alignment"],
    lastActivity: "4 hours ago",
    value: "$45.8M",
    progress: 50,
  },
  {
    id: "SBER-IND-RU-2026-007",
    title: "Generic Pharmaceuticals — Ministry of Health to Dr. Reddy's",
    sector: "Pharmaceuticals",
    parties: "Russian Ministry of Health, Dr. Reddy's Laboratories, Sberbank",
    status: "action_needed",
    riskLevel: "Medium",
    confidenceScore: 0.83,
    bottlenecks: ["Certification and local production setup", "INR/RUB invoice settlement"],
    suggestedTasks: ["Support registration with Russian regulators", "Structure trade finance for localization"],
    lastActivity: "1 day ago",
    value: "$24.3M",
    progress: 45,
  },
  {
    id: "SBER-IND-RU-2026-008",
    title: "Food Exports — Main Chain LLC to Greenfields Agro",
    sector: "Agriculture / Food",
    parties: "Main Chain LLC, Greenfields Agro Exports, Sberbank",
    status: "on_track",
    riskLevel: "Low",
    confidenceScore: 0.85,
    bottlenecks: ["Tariff paperwork coordination", "Inventory planning for demand rise"],
    suggestedTasks: ["Support partner search for rice/buffalo suppliers", "Enable cross-border payment routing"],
    lastActivity: "6 hours ago",
    value: "$9.6M",
    progress: 65,
  },
  {
    id: "SBER-IND-RU-2026-009",
    title: "Ceramic Tiles — Russian Builder Corp to Somany Ceramics",
    sector: "Manufacturing / Ceramics",
    parties: "Russian Builder Corp, Somany Ceramics, Sberbank",
    status: "action_needed",
    riskLevel: "Medium",
    confidenceScore: 0.68,
    bottlenecks: ["Customs classification delays", "Payment timing for high-volume imports"],
    suggestedTasks: ["Provide tariff classification support", "Structure letter of credit facilities"],
    lastActivity: "2 days ago",
    value: "$11.2M",
    progress: 30,
  },
  {
    id: "SBER-IND-RU-2026-010",
    title: "E-Commerce Spices — Moscow E-Market to SpiceKart India",
    sector: "E-Commerce / Consumer Goods",
    parties: "Moscow E-Market Ltd, SpiceKart India, Sberbank",
    status: "on_track",
    riskLevel: "Low",
    confidenceScore: 0.79,
    bottlenecks: ["Cross-border parcel payment reconciliation"],
    suggestedTasks: ["Enable end-to-end payment processing", "Support e-marketplace settlement reconciliation"],
    lastActivity: "5 hours ago",
    value: "$7.4M",
    progress: 60,
  },
];

export const MOCK_MESSAGES: Message[] = [
  {
    id: "1",
    sender: "Rajesh Kumar (Tata Steel)",
    content: "Hi team, the LC amendment has been submitted to Sber BD. Expected processing time is 3 business days. We've also shipped the first batch documentation.",
    timestamp: "2024-03-15 09:30",
    type: "email",
    dealId: "DEAL-IR-2024-001",
    sentiment: "positive",
  },
  {
    id: "2",
    sender: "Ivan Petrov (Severstal)",
    content: "Спасибо, Раджеш. Мы проверили документацию — всё в порядке. Наша логистическая команда готова координировать портовые операции, как только получим подтверждение.",
    timestamp: "2024-03-15 10:15",
    type: "email",
    dealId: "DEAL-IR-2024-001",
    sentiment: "positive",
  },
  {
    id: "3",
    sender: "Priya Sharma (Sber BD India)",
    content: "LC amendment is under review. There's a minor discrepancy in the commodity description—please update clause 47A to match the revised proforma invoice.",
    timestamp: "2024-03-15 11:00",
    type: "email",
    dealId: "DEAL-IR-2024-001",
    sentiment: "neutral",
  },
  {
    id: "4",
    sender: "AI Agent (Scribe)",
    content: "📋 Communication Summary: 3 emails processed. Key topics: LC amendment status, documentation review, minor discrepancy flagged in commodity description. All parties responsive.",
    timestamp: "2024-03-15 11:05",
    type: "system",
    dealId: "DEAL-IR-2024-001",
    sentiment: "neutral",
  },
  {
    id: "5",
    sender: "AI Agent (Analyst)",
    content: "📊 Deal Health: ON TRACK | Confidence: 92% | Risk: Low\n\nThe LC amendment process is proceeding normally. The discrepancy flagged by Sber BD is routine and should be resolved within 24hrs. No blocking issues detected.",
    timestamp: "2024-03-15 11:06",
    type: "system",
    dealId: "DEAL-IR-2024-001",
    sentiment: "positive",
  },
  {
    id: "6",
    sender: "AI Agent (Strategist)",
    content: "🎯 Recommended Action: Update clause 47A in proforma invoice to match LC terms. Assign to: Rajesh Kumar. Deadline: March 16.\n\n📝 Draft email prepared for RM review.",
    timestamp: "2024-03-15 11:07",
    type: "system",
    dealId: "DEAL-IR-2024-001",
    sentiment: "neutral",
  },
  {
    id: "42",
    sender: "Rajesh Kumar (Tata Steel)",
    content: "Priya, clause 47A has been updated. Revised proforma invoice attached. Ivan, please note: the first shipment of 1,200 MT of HR coils is scheduled for loading at Nhava Sheva on March 22. We need your Novorossiysk port agent details for the Bill of Lading.",
    timestamp: "2024-03-16 09:45",
    type: "email",
    dealId: "DEAL-IR-2024-001",
    sentiment: "positive",
  },
  {
    id: "43",
    sender: "Ivan Petrov (Severstal)",
    content: "Раджеш, наш портовый агент в Новороссийске — ООО «МорТрансСервис». Контакт: Дмитрий Козлов, +7-861-261-XXXX. Он уже проинформирован о поставке. Просьба указать в коносаменте следующее: грузополучатель — ПАО «Северсталь», Череповец, Россия.",
    timestamp: "2024-03-16 11:30",
    type: "email",
    dealId: "DEAL-IR-2024-001",
    sentiment: "positive",
  },
  {
    id: "44",
    sender: "Priya Sharma (Sber BD India)",
    content: "Clause 47A discrepancy is now resolved. The LC amendment has been approved and operative. Reference: SBER/LC/2024/IR-001-AMD-02. Rajesh, please ensure all shipping documents reference this amended LC number.",
    timestamp: "2024-03-16 14:20",
    type: "email",
    dealId: "DEAL-IR-2024-001",
    sentiment: "positive",
  },
  {
    id: "45",
    sender: "Ivan Petrov (Severstal)",
    content: "Отлично, аккредитив подтверждён. С нашей стороны всё готово для приёмки груза. Вопрос по таможне: у нас новые требования по сертификату происхождения для стальной продукции из Индии. Необходим сертификат формы CT-1, заверенный Торгово-промышленной палатой Индии. Без него таможенное оформление в России будет задержано.",
    timestamp: "2024-03-17 08:15",
    type: "chat",
    dealId: "DEAL-IR-2024-001",
    sentiment: "neutral",
  },
  {
    id: "46",
    sender: "Rajesh Kumar (Tata Steel)",
    content: "Ivan, noted on the CT-1 certificate requirement. We are processing it with the Federation of Indian Chambers of Commerce (FICCI). Should be ready within 2 business days. Meanwhile, our shipping line has confirmed vessel MV Stellar Fortune, ETD Nhava Sheva: March 22, ETA Novorossiysk: April 8.",
    timestamp: "2024-03-17 10:00",
    type: "email",
    dealId: "DEAL-IR-2024-001",
    sentiment: "positive",
  },
  {
    id: "47",
    sender: "Vikram Singh (Sber BD)",
    content: "Team, quick update from Sberbank side. We've confirmed the INR-RUB settlement pathway for this transaction. Tata Steel will receive payment in INR via Sber BD India, and Severstal's RUB payment will be routed through Sberbank's correspondent account. FX rate will be locked 48 hours before vessel departure.",
    timestamp: "2024-03-17 12:30",
    type: "chat",
    dealId: "DEAL-IR-2024-001",
    sentiment: "positive",
  },
  {
    id: "48",
    sender: "Ivan Petrov (Severstal)",
    content: "Викрам, спасибо за подтверждение платёжного маршрута. Наш финансовый отдел согласовал фиксацию курса за 48 часов до отправки. Также прошу уточнить: комиссия Сбербанка за конвертацию включена в согласованный курс или взимается отдельно?",
    timestamp: "2024-03-17 14:00",
    type: "chat",
    dealId: "DEAL-IR-2024-001",
    sentiment: "neutral",
  },
  {
    id: "49",
    sender: "Vikram Singh (Sber BD)",
    content: "Ivan, the Sberbank conversion fee is 0.15% and is included in the quoted rate — no separate charge. I'll send the formal rate sheet once the lock window opens on March 20.",
    timestamp: "2024-03-17 14:45",
    type: "chat",
    dealId: "DEAL-IR-2024-001",
    sentiment: "positive",
  },
  {
    id: "50",
    sender: "Ivan Petrov (Severstal)",
    content: "Обновление по логистике: портовый агент Дмитрий подтвердил причал №7 в Новороссийске для разгрузки 8 апреля. Кран грузоподъёмностью 50 тонн зарезервирован. Нам также потребуется фитосанитарный контроль — это стандартная процедура для стальных грузов. Ожидаемое время оформления — 2 рабочих дня после прибытия.",
    timestamp: "2024-03-18 09:00",
    type: "email",
    dealId: "DEAL-IR-2024-001",
    sentiment: "positive",
  },
  {
    id: "51",
    sender: "Rajesh Kumar (Tata Steel)",
    content: "All documentation is now in order: amended LC, CT-1 certificate, commercial invoice, packing list, and insurance certificate. Loading at Nhava Sheva begins March 22 as planned. Ivan, I'll share the Bill of Lading and draft shipping documents for your review by March 23.",
    timestamp: "2024-03-18 11:30",
    type: "email",
    dealId: "DEAL-IR-2024-001",
    sentiment: "positive",
  },
  {
    id: "7",
    sender: "Dr. Ananya Patel (Sun Pharma)",
    content: "URGENT: CDSCO has requested additional documentation for the API export license. We need to submit revised stability data by end of week or face a 30-day delay.",
    timestamp: "2024-03-15 08:00",
    type: "email",
    dealId: "DEAL-IR-2024-002",
    sentiment: "negative",
  },
  {
    id: "8",
    sender: "Alexei Volkov (R-Pharm)",
    content: "Это вызывает серьёзную обеспокоенность. Задержка в 30 дней существенно повлияет на наш график дистрибуции. Можем ли мы рассмотреть параллельные маршруты импорта через третью страну?",
    timestamp: "2024-03-15 09:30",
    type: "email",
    dealId: "DEAL-IR-2024-002",
    sentiment: "negative",
  },
  {
    id: "9",
    sender: "AI Agent (Analyst)",
    content: "⚠️ Deal Health: ACTION NEEDED | Confidence: 74% | Risk: Medium\n\nRegulatory bottleneck detected. CDSCO approval is on the critical path. Delay probability: 65%. Recommending escalation and parallel planning.",
    timestamp: "2024-03-15 09:35",
    type: "system",
    dealId: "DEAL-IR-2024-002",
    sentiment: "negative",
  },
  // Synthetic Data Messages
  {
    id: "10",
    sender: "Alexei Volkov (BioPharma Mumbai)",
    content: "We cannot proceed with the shipment until the currency conversion rate for the INR payment is locked in at the rate discussed last week.",
    timestamp: "2026-03-14 09:15",
    type: "email",
    dealId: "IND-RUS-2026-0042",
    sentiment: "negative",
  },
  {
    id: "11",
    sender: "Maria Kuznetsova (MedCorp Moscow)",
    content: "Курс Центробанка снова изменился. Нам необходимо, чтобы Сбербанк подключился и предложил инструмент хеджирования для завершения этого транша.",
    timestamp: "2026-03-14 11:30",
    type: "email",
    dealId: "IND-RUS-2026-0042",
    sentiment: "negative",
  },
  {
    id: "12",
    sender: "Vikram Singh (Sber BD)",
    content: "Sberbank can offer our FX-Option-B hedging product to lock in your rates. Let's schedule a call tomorrow to finalize the structure.",
    timestamp: "2026-03-14 14:00",
    type: "chat",
    dealId: "IND-RUS-2026-0042",
    sentiment: "neutral",
  },
  {
    id: "13",
    sender: "Retail Manager (Main Chain LLC)",
    content: "Розничный рынок вырос на 15%, нам срочно нужны дженерики для обеспечения спроса на кардиоваскулярную терапию. Просьба поручить Сберу проверить ликвидность SRVA.",
    timestamp: "2026-03-14 10:00",
    type: "chat",
    dealId: "SBER-IND-RU-2026-002",
    sentiment: "positive",
  },
  {
    id: "14",
    sender: "Shweta Patel (Sun Pharma)",
    content: "We're expediting the CDSCO documentation. Expect submission by Friday. In parallel, Sber BD is fast-tracking the eBRC issuance.",
    timestamp: "2026-03-14 11:30",
    type: "email",
    dealId: "SBER-IND-RU-2026-002",
    sentiment: "positive",
  },
  {
    id: "15",
    sender: "Logistics Officer (LLC Unifrost)",
    content: "Сбер обеспечивает страхование на участке железнодорожного транзита через Казахстан?",
    timestamp: "2026-03-13 14:00",
    type: "chat",
    dealId: "SBER-IND-RU-2026-003",
    sentiment: "neutral",
  },
  {
    id: "16",
    sender: "Trade Finance Specialist (Sberbank)",
    content: "Yes, we provide trade finance support. Note: Shipments via Astrakhan are currently facing transshipment shortages; we recommend the direct rail link via Incheh Borun.",
    timestamp: "2026-03-13 15:00",
    type: "chat",
    dealId: "SBER-IND-RU-2026-003",
    sentiment: "positive",
  },
  {
    id: "17",
    sender: "SEZ Specialist (Stupino Kvadrat)",
    content: "Индийская команда Сбер обрабатывает перевод AD Code для отгрузочных накладных на оборудование. Также мы проверяем двух местных дистрибьюторов для ваших барьерных плёнок.",
    timestamp: "2026-03-12 16:00",
    type: "chat",
    dealId: "SBER-IND-RU-2026-004",
    sentiment: "positive",
  },
  {
    id: "18",
    sender: "Supply Chain Officer (Tata Steel)",
    content: "Per our contract, we agreed on USD pricing with an INR settlement at 82.50. Please confirm Sberbank will execute the final currency conversion.",
    timestamp: "2026-03-15 08:30",
    type: "email",
    dealId: "SBER-IND-RU-2026-005",
    sentiment: "neutral",
  },
  {
    id: "19",
    sender: "Compliance Manager (Northern Steel)",
    content: "Мы инициировали перевод в рублях на корреспондентский счёт Сбербанка. Убедитесь, что средства будут зачислены на рупийный счёт Tata Steel.",
    timestamp: "2026-03-15 09:20",
    type: "email",
    dealId: "SBER-IND-RU-2026-005",
    sentiment: "positive",
  },
  {
    id: "20",
    sender: "Crude Oil Trader (Reliance)",
    content: "As agreed, we will supply 500,000 bpd to Reliance under the new ruble settlement structure. Please confirm the Sberbank–Gazprombank corridor is ready.",
    timestamp: "2026-03-15 12:00",
    type: "email",
    dealId: "SBER-IND-RU-2026-006",
    sentiment: "neutral",
  },
  {
    id: "21",
    sender: "Pharma Director (Ministry of Health)",
    content: "Dr. Reddy's готов расширить локальное производство на калужском предприятии. Просим проконсультировать по структуре торгового финансирования Сбербанка для локализации.",
    timestamp: "2026-03-16 09:00",
    type: "email",
    dealId: "SBER-IND-RU-2026-007",
    sentiment: "positive",
  },
  {
    id: "22",
    sender: "Agri Export Officer (Main Chain LLC)",
    content: "Российские розничные сети запрашивают увеличенные объёмы риса и буйволового мяса. Просьба обеспечить подбор новых партнёров по дистрибуции через Сбербанк.",
    timestamp: "2026-03-17 08:00",
    type: "email",
    dealId: "SBER-IND-RU-2026-008",
    sentiment: "positive",
  },
  {
    id: "23",
    sender: "Builder Procurement (Russian Builder Corp)",
    content: "Нам нужна надёжная платёжная структура для предстоящего заказа керамической плитки. Просьба подтвердить условия аккредитива и фиксацию курса.",
    timestamp: "2026-03-17 09:15",
    type: "email",
    dealId: "SBER-IND-RU-2026-009",
    sentiment: "neutral",
  },
  {
    id: "24",
    sender: "E-Commerce Manager (Moscow E-Market)",
    content: "На нашей платформе резко вырос спрос на специи и аюрведическую косметику. Нам нужно, чтобы Сбербанк наладил платёжный поток: покупатели платят в рублях, а мы получаем в рупиях.",
    timestamp: "2026-03-17 10:00",
    type: "chat",
    dealId: "SBER-IND-RU-2026-010",
    sentiment: "positive",
  },
  {
    "id": "25",
    "sender": "Anil Deshmukh (L&T)",
    "content": "Dear Mikhail, Please find attached the revised commercial proposal for the Heavy Duty Heat Exchangers and Distillation Columns as discussed. The total contract value stands at $22.5M. We have incorporated the technical modifications requested by Elena. To ensure absolute clarity before we proceed to the formal contracting phase, I am outlining the comprehensive technical and commercial scope below for your immediate reference. \n\n1.0 EQUIPMENT OVERVIEW AND SCOPE OF SUPPLY: The scope of supply encompasses the complete design, engineering, procurement of raw materials, manufacturing, fabrication, assembly, inspection, testing, painting, packaging, and delivery (on CIF Novorossiysk terms) of three (3) Heavy Duty Shell and Tube Heat Exchangers (Item Tags: HX-201A/B/C) and two (2) Main Atmospheric Distillation Columns (Item Tags: DC-101, DC-102) intended for the Saratov refinery modernization project. The equipment is specifically engineered to handle high-sulfur Ural crude derivatives under extreme pressure and temperature variations. \n\n2.0 DETAILED TECHNICAL SPECIFICATIONS - HEAT EXCHANGERS (HX-201A/B/C): Each unit is designed in strict accordance with TEMA R (Tubular Exchanger Manufacturers Association, Class R for severe requirements of petroleum and related processing applications) and ASME Boiler and Pressure Vessel Code, Section VIII, Division 1. The shell side is rated for a design pressure of 45.0 kg/cm2(g) and a design temperature of 350°C. The tube side is rated for a design pressure of 60.0 kg/cm2(g) and a design temperature of 400°C. Metallurgy: Shell material shall be SA-516 Gr. 70 with a 3mm Hydrogen Induced Cracking (HIC) resistant cladding of SA-240 Type 316L. The tubes shall be seamlessly drawn, manufactured from Titanium Grade 2 (SB-338 Gr. 2) to ensure maximum resistance against naphthenic acid corrosion and sulfidation at elevated temperatures. Tube sheet material is SA-765 Gr. II with explosive bonded Titanium cladding. Baffle plates will be SA-516 Gr. 70. Each unit will feature a customized impingement baffle design to mitigate flow-induced vibration, which was a critical concern raised during the preliminary HAZOP review. The total heat transfer area per unit is exactly 1,450 square meters. \n\n3.0 DETAILED TECHNICAL SPECIFICATIONS - DISTILLATION COLUMNS (DC-101, DC-102): The main atmospheric columns have an overall tangent-to-tangent length of 48.5 meters and an internal diameter of 4.2 meters. Design pressure is 5.5 kg/cm2(g) / Full Vacuum, and design temperature is 380°C. The base material of construction is SA-516 Gr. 70 NACE (normalized) to meet NACE MR0103 standards for sulfide stress cracking resistance in corrosive petroleum refining environments. The internal cladding from the bottom tangent line up to the heavy gas oil draw-off tray (Tray #15) shall be 3mm thick Monel 400 (SB-127) to combat high-temperature hydrochloric acid and naphthenic acid corrosion. Above Tray #15 to the top tangent line, the cladding shall be 3mm thick SA-240 Type 410S stainless steel. The columns will be supplied fully dressed with internal tray support rings, downcomer bolting bars, and all necessary external platforms, ladders, and pipe supports. The scope excludes the actual fractionating trays, which we understand will be sourced separately by Rosneft from Koch-Glitsch. \n\n4.0 INSPECTION AND TESTING PROTOCOLS (ITP): The manufacturing process will be subject to a rigorous Inspection and Test Plan (ITP), requiring stage-wise approval from both L&T's internal QA/QC department and Rosneft's designated third-party inspection agency (TPIA). Required non-destructive testing (NDT) includes 100% Radiographic Testing (RT) of all long and girth seams on the shell and columns, 100% Ultrasonic Testing (UT) of all plate materials thicker than 25mm, Magnetic Particle Inspection (MPI) or Liquid Penetrant Inspection (LPI) of all fillet welds and attachment welds, and Positive Material Identification (PMI) on 100% of all alloy components, including tubes, cladding, and welding consumables. Furthermore, hydrostatic testing will be performed at 1.3 times the maximum allowable working pressure (MAWP) corrected for temperature, utilizing demineralized water with a chloride content not exceeding 25 ppm. A stringent helium leak test will also be conducted on the tube-to-tubesheet joints of the heat exchangers. \n\n5.0 COMMERCIAL TERMS AND DELIVERY SCHEDULE: As previously agreed, the total firm and fixed price is $22,500,000 USD. Payment terms are strictly via an irrevocable, unconfirmed, non-transferable Letter of Credit (LC) payable at sight, covering 100% of the contract value. The LC must be issued by a mutually acceptable international bank and advised through Sber BD India. The delivery timeline is set at 45 weeks from the date of receipt of the operative LC and approval of all major fabrication drawings. Delivery terms are CIF (Cost, Insurance, and Freight) to the Port of Novorossiysk, Russia, as per Incoterms 2020. \n\n6.0 DOCUMENTATION AND CERTIFICATION: L&T will provide a comprehensive Manufacturing Data Record (MDR) comprising, but not limited to: Approved as-built drawings, design calculations (including wind and seismic analysis for the columns), material test certificates (MTCs) as per EN 10204 Type 3.1 or 3.2 where applicable, welding procedure specifications (WPS), procedure qualification records (PQR), welder performance qualifications (WPQ), NDT reports, heat treatment charts, dimensional inspection reports, and the final release note from the TPIA. We also require a formalized End-User Certificate (EUC) from Rosneft explicitly stating the civilian application of these goods, which is a mandatory prerequisite for our internal compliance and export control procedures. \n\n7.0 WARRANTY AND LIABILITY: L&T warrants that the equipment shall be free from defects in material and workmanship for a period of 18 months from the date of shipment or 12 months from the date of commissioning, whichever occurs first. Our maximum aggregate liability under this contract, whether in contract, tort, or otherwise, shall be strictly limited to 100% of the contract value. Consequential, indirect, or punitive damages are explicitly excluded. \n\nPlease review this exhaustive summary. If everything is in order, we can proceed to finalize the overarching commercial contract and simultaneously begin the LC drafting process. We are eager to commence engineering works and secure the raw material supply chain, which is currently experiencing significant lead-time volatility in the global market. Looking forward to your prompt confirmation.",
    "timestamp": "2024-02-10 09:00",
    "type": "email",
    "dealId": "DEAL-IR-2024-003",
    "sentiment": "positive"
  },
  {
    "id": "26",
    "sender": "Mikhail Volkov (Rosneft)",
    "content": "Анил, получено. Цена нас устраивает. Команда Елены проводит финальную проверку. Тем временем, пожалуйста, начните процесс оформления аккредитива с вашими банковскими партнёрами. Мы хотим действовать быстро.",
    "timestamp": "2024-02-11 14:30",
    "type": "email",
    "dealId": "DEAL-IR-2024-003",
    "sentiment": "positive"
  },
  {
    "id": "27",
    "sender": "Elena Smirnova (Rosneft)",
    "content": "Подтверждаю техническое одобрение с нашей стороны. Спецификации металлургии для среды с высоким содержанием серы были корректно обновлены. Переходим к следующим шагам.",
    "timestamp": "2024-02-12 11:15",
    "type": "email",
    "dealId": "DEAL-IR-2024-003",
    "sentiment": "positive"
  },
  {
    "id": "28",
    "sender": "Anil Deshmukh (L&T)",
    "content": "Adding Rohan Mehta from Sber BD India to this thread. Rohan, please see the attached proforma invoice for $22.5M. We need to initiate the LC issuance process. Rosneft will be the applicant. Let us know the required documentation.",
    "timestamp": "2024-02-12 16:45",
    "type": "email",
    "dealId": "DEAL-IR-2024-003",
    "sentiment": "neutral"
  },
  {
    "id": "29",
    "sender": "Rohan Mehta (Sber BD India)",
    "content": "Thanks Anil. Congratulations on the mandate. I've forwarded the PI to our trade finance desk. However, given the destination and current geopolitical climate, I also have to loop in our Global Compliance Team. We will need comprehensive End-User Certificates (EUC) and a detailed routing map.",
    "timestamp": "2024-02-13 10:05",
    "type": "email",
    "dealId": "DEAL-IR-2024-003",
    "sentiment": "neutral"
  },
  {
    "id": "30",
    "sender": "Anil Deshmukh (L&T)",
    "content": "Understood Rohan. Mikhail, could you please provide the standard EUC format filled out for this specific site? Also, we are planning to ship via Novorossiysk port.",
    "timestamp": "2024-02-13 11:30",
    "type": "email",
    "dealId": "DEAL-IR-2024-003",
    "sentiment": "neutral"
  },
  {
    "id": "31",
    "sender": "Mikhail Volkov (Rosneft)",
    "content": "Во вложении сертификат конечного пользователя. Оборудование предназначено исключительно для гражданских процессов нефтепереработки на нашем Саратовском объекте. Никакого двойного назначения. Просьба ускорить оформление аккредитива — сроки поджимают.",
    "timestamp": "2024-02-15 08:45",
    "type": "email",
    "dealId": "DEAL-IR-2024-003",
    "sentiment": "neutral"
  },
  {
    "id": "32",
    "sender": "Neha Gupta (Sber BD Compliance)",
    "content": "Hi all, Compliance here. We have reviewed the EUC. Unfortunately, this transaction has been flagged by our automated screening system. We require an exhaustive Sanctions Compliance Review. This means we need full disclosure of all ultimate beneficial owners (UBOs) of the importing entity, and a legal opinion confirming this specific HS code is not restricted.",
    "timestamp": "2024-02-18 14:20",
    "type": "email",
    "dealId": "DEAL-IR-2024-003",
    "sentiment": "negative"
  },
  {
    "id": "33",
    "sender": "Vikram Singh (L&T Legal)",
    "content": "Dear Neha and the Sber BD Compliance Team, \n\nMEMORANDUM\nSUBJECT: Preliminary Sanctions Risk Assessment and Legal Analysis for Export Transaction (DEAL-IR-2024-003) - L&T to Rosneft (Saratov Refinery)\n\n1.0 INTRODUCTION AND PURPOSE: This memorandum has been prepared by the internal legal counsel of Larsen & Toubro (L&T) to address the compliance flags raised by Sber BD India concerning the proposed export of Heavy Duty Heat Exchangers and Atmospheric Distillation Columns to Rosneft for use in their Saratov oil refinery in Russia. The purpose of this exhaustive analysis is to demonstrate that the transaction in question does not violate current applicable sanctions regimes, specifically those enforced by the United States Office of Foreign Assets Control (OFAC), the European Union (EU), and the United Kingdom's Office of Financial Sanctions Implementation (OFSI), thus warranting the immediate removal of the compliance block on the Letter of Credit (LC) issuance. \n\n2.0 FACTUAL BACKGROUND AND ENTITY SCREENING: The exporter is Larsen & Toubro, an Indian multinational conglomerate. The importer is Rosneft, a Russian integrated energy company. The end-use destination is the Saratov Oil Refinery, a civilian industrial facility. We acknowledge that Rosneft is currently subject to various sanctions directives.",
    "timestamp": "2024-02-20 16:10",
    "type": "email",
    "dealId": "DEAL-IR-2024-003",
    "sentiment": "neutral"
  },
  {
    "id": "34",
    "sender": "Neha Gupta (Sber BD Compliance)",
    "content": "Vikram, thank you for the memo, but it is insufficient. Our correspondent banking partners require an independent third-party legal counsel opinion, not an internal one. Furthermore, the compliance hold remains firmly in place until this is resolved.",
    "timestamp": "2024-02-22 09:30",
    "type": "email",
    "dealId": "DEAL-IR-2024-003",
    "sentiment": "negative"
  },
  {
    "id": "35",
    "sender": "Mikhail Volkov (Rosneft)",
    "content": "Анил, что происходит? Мы ждём проект аккредитива уже больше недели. График проекта срывается. Если Sber BD India не в состоянии это обработать, нам нужно знать немедленно, чтобы перейти на механизм в другой валюте или найти другого поставщика.",
    "timestamp": "2024-02-25 13:45",
    "type": "email",
    "dealId": "DEAL-IR-2024-003",
    "sentiment": "negative"
  },
  {
    "id": "36",
    "sender": "Anil Deshmukh (L&T)",
    "content": "Mikhail, we sincerely apologize for the delay. We are escalating this internally and with Sber BD. Rohan, can we get an exception or a faster track on the third-party legal review? The deal is at serious risk.",
    "timestamp": "2024-02-25 14:15",
    "type": "email",
    "dealId": "DEAL-IR-2024-003",
    "sentiment": "negative"
  },
  {
    "id": "37",
    "sender": "Rohan Mehta (Sber BD India)",
    "content": "Anil, Vikram, Mikhail. \n\nURGENT UPDATE: Logistics, Marine Insurance Risk Assessment, and LC Feasibility. \n\nI am writing to formally escalate a critical secondary bottleneck that has emerged from our logistics risk desk, independent of the ongoing sanctions compliance review. Even if the Baker McKenzie legal opinion clears the sanctions hurdle, we are facing an insurmountable barrier regarding the marine insurance and logistics framework required to operationalize this Letter of Credit. This constitutes a severe Insurance Coverage Gap that fundamentally compromises the viability of a CIF Novorossiysk delivery term under a bank-intermediated trade structure. I need to explain the depth of this issue so all parties understand why the transaction remains blocked. \n\n1.0 CURRENT INSURANCE MARKET DYNAMICS FOR THE BLACK SEA: The marine insurance market for voyages entering the Black Sea, specifically concerning Russian ports, has hardened to an unprecedented degree. Following the continued geopolitical volatility in the region, the Joint War Committee (JWC) of the Lloyd's Market Association (LMA) and the International Underwriting Association (IUA) has maintained the Russian Black Sea and Sea of Azov coastlines on its list of Hull War, Piracy, Terrorism and Related Perils Listed Areas. This designation radically alters the standard insurance landscape. \n\n2.0 WITHDRAWAL OF STANDARD COVER AND WAR RISK PREMIUMS: Historically, L&T has utilized standard Institute Cargo Clauses (A) policies covering all risks of loss or damage. However, nearly all major global reinsurers and primary marine insurers operating in London, Europe, and Asia have formally withdrawn or severely curtailed their coverage for shipments destined for Russian ports. They are invoking sweeping exclusionary clauses relating to war, strikes, riots, and civil commotions (SRCC). \n\nFor the very few specialist syndicates willing to write a policy for a $22.5M cargo to Novorossiysk, the War Risk Premiums (WRP) are astronomical. Instead of a standard fraction of a percent, we are seeing quotes ranging from 5% to 10% of the cargo value, valid only for 48 to 72 hours before the vessel enters the high-risk zone. The cost of this insurance alone could exceed $2 million, completely destroying the commercial margins of this contract. Furthermore, these policies contain stringent \"cancellation clauses\" allowing the insurer to terminate coverage with merely 48 hours' notice if the security situation deteriorates, leaving the cargo effectively uninsured mid-transit. \n\n3.0 P&I CLUB RESTRICTIONS: Beyond the cargo insurance, there is the issue of the vessel itself. Protection and Indemnity (P&I) clubs, which provide third-party liability cover for shipowners, are incredibly hesitant to cover vessels calling at Novorossiysk. Without valid P&I cover, major shipping lines (Maersk, MSC, Hapag-Lloyd) flatly refuse to accept the booking. The pool of available vessels is reduced to a shadow fleet of aging bulk carriers and break-bulk vessels that do not meet the safety and vetting standards required by Sber BD India's internal trade finance policies for a high-value capital goods shipment. \n\n4.0 PORT OF NOVOROSSIYSK OPERATIONAL RISKS: Novorossiysk is a major naval base as well as a commercial port. Our logistics desk assesses a high probability of collateral damage, port closures, or unannounced naval blockades. If the vessel is delayed at anchorage for weeks due to military traffic or security alerts, who bears the exorbitant demurrage costs? The current CIF contract structure places this risk heavily on the supplier (L&T) until the goods cross the ship's rail at the destination port, but under an LC structure, any delay in presenting clean on-board bills of lading compromises the payment mechanism. \n\n5.0 IMPACT ON LETTER OF CREDIT FORMULATION: An LC is a documentary credit. Under UCP 600 rules, the bank deals in documents, not goods. To negotiate a CIF LC, L&T must present an insurance document that appears on its face to cover the risks stipulated in the LC. Given the current market, it is practically impossible for L&T to secure an insurance certificate that covers \"All Risks\" including war and strikes without massive exclusionary caveats that would render the document discrepant. \n\nIf Sber BD issues an LC demanding standard insurance, and L&T presents a restricted policy, the presentation is discrepant, payment is refused, and the LC fails. Sber BD India cannot deliberately issue an LC knowing that the beneficiary cannot possibly fulfill the documentary requirements regarding insurance. It is a violation of our fiduciary duty and internal risk controls. \n\n6.0 ALTERNATIVE PATHWAYS (FOB VS CIF): The only mathematically and operationally viable solution from the Bank's perspective is to fundamentally restructure the Incoterms. The contract must be changed from CIF Novorossiysk to FOB (Free On Board) or FCA (Free Carrier) at an Indian port (e.g., Mumbai/Nhava Sheva). \n\nUnder an FOB structure, Rosneft, as the buyer, assumes the responsibility and cost of chartering the vessel and securing the marine and war risk insurance. Rosneft likely has access to domestic Russian insurance markets (e.g., Ingosstrakh, SOGAZ) or state-backed guarantees that western and Indian banks cannot access. L&T's responsibility would end upon loading the heavy equipment onto the vessel in India. The LC would then be contingent upon presentation of a Bill of Lading showing loading in India, rather than relying on a highly fragile Black Sea transit. \n\nI understand Mikhail previously stated that switching to FOB requires a complete contract rewrite and board approval taking 3 weeks. However, I must be unequivocally clear: Sber BD India will not—and functionally cannot—issue a CIF Letter of Credit for a $22.5 million cargo to a Russian Black Sea port under the current global insurance collapse. The coverage gap is too vast, the risk to the underlying asset is too high, and the documentary requirements of the LC cannot be reliably met. The sanctions review is moving forward, but without resolving this logistics and insurance paralyzation, the deal remains definitively blocked.",
    "timestamp": "2024-02-26 10:00",
    "type": "email",
    "dealId": "DEAL-IR-2024-003",
    "sentiment": "negative"
  },
  {
    "id": "38",
    "sender": "Anil Deshmukh (L&T)",
    "content": "This is disastrous. We cannot ship $22.5M worth of equipment without comprehensive marine and war-risk insurance. Rohan, does Sber BD have any alternative insurer brokers who specialize in high-risk corridors? Mikhail, can Rosneft potentially arrange insurance on your end?",
    "timestamp": "2024-02-26 11:20",
    "type": "email",
    "dealId": "DEAL-IR-2024-003",
    "sentiment": "negative"
  },
  {
    "id": "39",
    "sender": "Mikhail Volkov (Rosneft)",
    "content": "Переход на FOB сейчас требует полной переделки контракта и одобрения совета директоров, на что уйдёт 3 недели. Неприемлемо. Вы котировали CIF — будьте добры обеспечить CIF. Мы ожидаем, что вы немедленно найдёте альтернативного страховщика.",
    "timestamp": "2024-02-27 09:05",
    "type": "email",
    "dealId": "DEAL-IR-2024-003",
    "sentiment": "negative"
  },
  {
    "id": "40",
    "sender": "Vikram Singh (L&T Legal)",
    "content": "Just to update everyone: we have engaged external counsel (Baker McKenzie) to provide the independent sanctions opinion requested by Sber BD. They estimate 7-10 days for completion. Regarding the insurance, our broker is checking with P&I clubs in the Middle East.",
    "timestamp": "2024-02-28 14:50",
    "type": "email",
    "dealId": "DEAL-IR-2024-003",
    "sentiment": "neutral"
  },
  {
    "id": "41",
    "sender": "Rohan Mehta (Sber BD India)",
    "content": "Noted Vikram. Please be advised that until both the sanctions compliance review is officially cleared by Neha's team, and a valid insurance certificate is presented, the status of this deal in our system is firmly blocked. Progress is frozen.",
    "timestamp": "2024-03-01 10:10",
    "type": "email",
    "dealId": "DEAL-IR-2024-003",
    "sentiment": "negative"
  }
];
